...
\quad The second year of my PhD was a continuation with the "Data-Centric Parallelization (DCP)" project. To work towards the recognition of data structures in legacy codes one would need to find the ground for that. In the previous year we conducted a feasibility study on some of the SPEC CPU2006 benchmarks and we learnt several things.       

\begin{description}[style=nextline]
\item [Research interest] Some of the SPEC CPU2006 benchmarks present an interest from the point of data structure and algorithmic skeleton recognition. For example, 429.mcf benchmark works with dynamic pointer-linked data structure. Benchmarks \hfill
\item [Real world legacy code complexity] SPEC CPU2006 benchmarks have proved to be extremely complex. For example, 429.mcf benchmark operates with a network of nodes and arcs laid out linearly in the heap memory. Despite simplicity of  allocation every node and arc structure has numerous pointers, which span several object linking chains. Operations with the data structure are spread all around the benchmark. It is extremely complex to grasp an algorithm and all the data structure details even by the means of manual source code study. Application of static techniques seems infeasible, while application of dynamic techniques is extremely complex.    




\hfill
\item [Close relationship between algorithms and data structures]\hfill
\item []\hfill
...
\item Thesis writing
\end{itemize}
\quad All above tasks are interdependent and happened iteratively and in close cooperation with one another. The process had to be repeated over and over until satisfactory (above 90\%) ML model predictive performance has been achieved.
...
1) A binary tree is built. Every node of the tree represents a city located on 2D plane with randomly generated (x,y) coordinates. The build_tree() method is written in a way to generate a uniform distribution of dots on the plane.
